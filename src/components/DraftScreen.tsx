          ...
          return (
            <div key={fighter.id} className="animate-deal-in min-h-0" style={{ animationDelay: `${index * 110}ms` }}>
              <button
                ...
                className={`group flex h-full w-full items-stretch overflow-hidden border text-left backdrop-blur-[3px] transition-all duration-200 sm:flex-col ${
                  selected
                    ? "animate-lock-in border-belt-gold bg-panel/50"
                    : busy
                      ? "border-line/70 bg-panel/20 opacity-40"
                      : "border-line/70 bg-panel/25 hover:-translate-y-1 hover:border-corner-red hover:bg-panel/45"
                }`}
              >
                <Avatar
                  name={fighter.name}
